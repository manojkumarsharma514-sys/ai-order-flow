...
2026-08-13: added compute_extras() and compute_calendar() for the
redesigned Analytics tab (win-rate gauge, profit factor, trade
expectancy, avg win/loss, net P&L sparkline, day/trade streaks, and a
month-view P&L calendar). Both are computed fresh on demand rather
than folded into compute()/recompute() — analytics_summary.csv is an
append-only audit log with a fixed column schema going back to this
app's first session, and adding new keys to the dict that
recompute() appends would silently misalign every future row's
columns against every row already on disk. Recomputing on demand costs
nothing extra beyond what compute() already does (both read the same
orders_history.csv) and keeps the persisted schema untouched.
"""
...
class AnalyticsEngine:
    ...
    # ------------------------------------------------------------
    # Extras for the redesigned Analytics tab (not persisted)
    # ------------------------------------------------------------

    def compute_extras(self) -> dict:
        """
        avg_win / avg_loss:  mean realized P&L of winning / losing
                              trades (avg_loss is negative or 0.0).
        trade_expectancy:    total_realized_pnl / total_trades — the
                              average $ result of a trade, win or lose.
        day_streak:          consecutive CALENDAR days, most recent
                              first, with positive net realized P&L.
                              Breaks (stops counting) at the first day
                              — going backwards — that wasn't positive.
        trade_streak:        consecutive individual winning trades,
                              most recent first. Breaks at the first
                              trade — going backwards — that wasn't a
                              win (loss OR breakeven).
        equity_curve:        cumulative realized P&L after each trade,
                              oldest to newest — feeds the Net P&L
                              sparkline.
        """

        df = self.orders_manager.load_all()
        if df.empty:
            return dict(EMPTY_EXTRAS)
        ...
        total_trades = len(df)
        trade_expectancy = float(df["pnl_usd"].sum() / total_trades) if total_trades else 0.0

        equity_curve = df["pnl_usd"].cumsum().tolist()

        trade_streak = 0
        for pnl in reversed(df["pnl_usd"].tolist()):
            if pnl > 0:
                trade_streak += 1
            else:
                break

        daily = df.dropna(subset=["closed_at"]).groupby(
            df["closed_at"].dt.date
        )["pnl_usd"].sum().sort_index()

        day_streak = 0
        for pnl in reversed(daily.tolist()):
            if pnl > 0:
                day_streak += 1
            else:
                break
    ...
    def compute_calendar(self, year: int, month: int) -> dict:
        """{day_of_month (int) -> {"pnl": float, "count": int}} for
        every closed trade in the given month — feeds the P&L
        Calendar grid. Days with no closed trades simply have no key."""
        ...
        in_month = df[(df["closed_at"].dt.year == year) & (df["closed_at"].dt.month == month)]
        if in_month.empty:
            return {}

        grouped = in_month.groupby(in_month["closed_at"].dt.day)["pnl_usd"].agg(["sum", "count"])

        return {
            int(day): {"pnl": round(float(row["sum"]), 2), "count": int(row["count"])}
